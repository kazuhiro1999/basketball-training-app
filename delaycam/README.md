...
スロー / LIVE / リプレイ / 骨格 / REC / 設定）を置き、鏡・回転・全画面・QR・ヘルプ・終了は「⚙ 設定」の中にまとめてある。
...
操作バーの **「⟲ リプレイ」**（または `I` キー）で、**押した時点から過去 30 秒**（設定で 10〜60 秒）の映像を
画面いっぱいのプレイヤーで見返せる。遅延表示にまだ出ていない最新の部分も含むので、遅延を待たずに直前のプレーを確認できる。

| 操作 | キー |
|---|---|
| 再生 / 一時停止（映像クリックでも可） | Space |
| 1 コマ戻す / 進める | ← / → |
| 2 秒戻す / 進める | PageUp / PageDown（Shift+← / →） |
| 先頭へ | Home |
| 速度 1× / 0.5× / 0.25× | S で切替 |
| ループ再生 | O |
| 骨格表示 | P |
...
シークバーをドラッグして好きな位置へ移動できる。鏡・回転・骨格の設定は遅延表示と同じものが効く
（骨格はアナライザが処理済みの区間に表示される）。

仕組みと負荷: 表示側はもともと直近 90 秒以上のエンコード済み映像をメモリに持っている（巻き戻し用）。
リプレイは押した時点でその範囲を切り出して専用のデコーダで再生するだけで、サーバや録画には触れない。
リプレイ中は**遅延表示の復号・描画だけを止め**、受信と再生位置は進め続けるので、閉じるとその時点の正しい
遅延位置から再開する（実測: 開く前 15.05 秒 → 閉じた直後 15.06 秒）。録画と骨格推定もそのまま続く。
...
| `static/replay.js` | インスタントリプレイのプレイヤー。view.js からは `open` / `isOpen` / `onKey` だけを呼ぶ |
