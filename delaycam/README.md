...
性能の目安（ノート PC i7-14650HX・他の負荷なしでの定常値。**同じ PC でゲーム等を動かすと 2〜3 倍遅くなる**）:

| モデル | 1 回あたり | 毎秒 | 30fps なら間隔 |
|---|---|---|---|
| 軽い（最大 2 人） | 約 35 ms | 約 28 回 | 1〜2 |
| 標準（最大 6 人） | 約 90 ms | 約 11 回 | 3 |
| 重い | 約 330 ms | 約 3 回 | 10 |

**「自動」にしておけば PC の速さに合わせて勝手に決まる**ので、まず自動で使い、骨格がカクつくなら
「軽い」に落とすのが簡単。フリースロー（1 人ずつ）なら「軽い」で毎フレーム近く推定できる。
