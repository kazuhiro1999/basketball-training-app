...
- プリセット（いずれも rtmpose 系）と、ノート PC（i7-14650HX・他の負荷なし、768×576、4〜6 人）での**定常**実測:

  | プリセット | モデル | 最大人数 | 人検出 | 1 回あたり | 毎秒 |
  |---|---|---|---|---|---|
  | 軽い `light` | rtmpose-s | 2 | 3 回に 1 回 | 35 ms | 約 28 回 |
  | 標準 `medium` | rtmpose-s | 6 | 毎回 | 90 ms | 約 11 回 |
  | 重い `heavy` | rtmpose-m | 6 | 毎回 | 330 ms | 約 3 回 |

- `max_persons` は大きく写る順に上位だけ姿勢推定して top-down の時間を抑える。
- `det_every` は人検出を N 回に 1 回だけ行い、間のフレームは**直前の検出枠の大きさのまま、胴体（肩・腰）の位置に合わせて動かした枠**を使う。
  人数が少ないほど効く（最大 2 人: 60→35 ms）が、最大 6 人では 90→78 ms と効果が小さい一方、
  人が交差したり急に遠ざかる時に ID が切れやすくなる（vtest で ID 数 12→15）ので、「軽い」だけで使っている。
  枠を骨格の外接矩形から作り直す以前の実装は、検出枠と大きさが違うために 1 推論ごとに枠が伸び縮みし
  （10% 超の変化が 64% のフレームで発生）、関節の揺れと ID 切れの原因になっていた
...
定常の実測（i7-14650HX ノート・他の負荷なし、768×576、4〜6 人、ORT 8 スレッド）:

| backend | 開始直後 | 定常 | 定常 fps | 拾えた人数 |
|---|---|---|---|---|
| rtmpose-s (最大2人, 検出1/3) | 37 ms | 35 ms | 28.5 | 2.0 |
| rtmpose-s (最大2人) | 59 ms | 60 ms | 16.7 | 2.0 |
| rtmpose-s (最大6人, 検出1/3) | 64 ms | 78 ms | 12.9 | 4.7 |
| rtmpose-s (最大6人) | 83 ms | 90 ms | 11.2 | 4.7 |
| rtmpose-m (最大6人) | 333 ms | 328 ms | 3.0 | 4.7 |
| rtmo-t | 38 ms | 43 ms | 23.4 | 2.1 |
| rtmo-s | 120 ms | 126 ms | 7.9 | 2.2 |
| yolo-n | 51 ms | 57 ms | 17.6 | 0.8 |

同じ PC でゲームを動かしながら測ると、これの 2〜3 倍遅くなった（CPU の取り合いと発熱によるクロック低下）。
遅延再生の PC で他の重い処理を同時に動かさないこと。
